...
// 	$Id: Function1d.h,v 1.3 2003-06-16 18:49:07 garyb Exp $	 
...
public:
  ...
  Polynomial(int _ord, vector<V>& cin): order(_ord) {
    if (cin.size()<order+1) 
      throw MyException("Input coefficient array too small for"
			" Polynomial");
  ...
  double partial(const A arg, int iparam) const {
    if (iparam<0 || iparam>order)
      throw MyException("Invalid parameter number in Polynomial");
    return pow(arg, static_cast<double> (iparam));
  }
  void Set(const V c, int iparam) {
    if (iparam<0 || iparam>order)
      throw MyException("Invalid parameter number in Polynomial.Set()");
    coeffs[iparam]=c;
  }
  V Get(int iparam) const {
    if (iparam<0 || iparam>order)
      throw MyException("Invalid parameter numeber in Polynomial.Get()");
