// $Id: odeint.cpp,v 1.2 2005-10-24 18:45:46 garyb Exp $
#include "odeint.h"
#include "Std.h"
	  ...
	  const ODEFunction& derivs)
{
  ...
  for (;;) {
    ...
    if (errmax > 1.0) {
      ...
      xnew=x+h;
      if (xnew == x) 
	throw MyException("stepsize underflow in rkqs");
      continue;
    } else {
	    ...
	    const ODEFunction& derivs)
{
  ...
  for (int nstp=0;nstp<MAXSTP;nstp++) {
    ...
    if (abs(hnext) <= hmin) 
      throw MyException("Step size too small in odeint");
    h=hnext;
  }
  throw MyException("Too many steps in routine odeint");
}
#undef MAXSTP
#undef TINY
