...
public:
  ...
  myvector<T>& operator[](uint i) {return data[i]; }
  const myvector<T>& operator[](uint i) const {return data[i];}
//operator T() const {Assert(getN1()==1&&getN2()==1); return data[0][0];}
  ...
  const myvector<T>& getRow(uint i) const
    {Assert(i<getN1()); return data[i]; }
  myvector<T> getCol(uint j) const
    {Assert(j<getN2()); 
      myvector<T> temp(getN1()); 
...
{
  if (n1 == 0) data = myvector<myvector<T> >(newn1,myvector<T>(n2));
  else if (newn1 > n1) data.insert(data.end(),newn1-n1,myvector<T>(n2));
...
{
  if (n2 == 0) for(uint i=0;i<n1;i++) data[i] = myvector<T>(newn2);
